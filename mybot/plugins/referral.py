...
async def withdraw_cb(client, callback_query):
    ...
    await callback_query.message.reply_text("Withdrawal request submitted. Admin will review it soon.", quote=True)
    await callback_query.answer()
...
async def mypoints_cb(client, callback_query):
    ...
    points = data.get("points", 0)
    await callback_query.message.reply_text(f"You currently have <b>{points}</b> points.", quote=True)
    await callback_query.answer()
